...
- the ai-search TYPO3 plugin is now able to handle conversations
- behaviour can be configured in the extension settings per plugin instance
- create production save chat session id and store in in local browser-storage to survive browser reloads and restarts

TODO: If you have a modified template that overwrites the EXT:rms-ai-search/Resources/Private/Templates/Search/Search.html template, add the following line

```<input type="hidden" id="aisearch-clear-chat-history" value="{aisearch_clear_chat_history}" />```

requires new js / css files if you do not use the TYPO3 extension
...
Backport dialog functionality from v2 (TYPO3 v13) version, see description of V2.3.0 for details
...
backend: fix bug where the configuration tab of rms-ai-search was also visible in other TYPO3-plugins
