<?php

declare(strict_types=1);

use TYPO3\CMS\Core\Utility\ExtensionManagementUtility;
use TYPO3\CMS\Extbase\Utility\ExtensionUtility;

defined('TYPO3') || die();

$contentElementType = 'rmsaisearch_search';

ExtensionUtility::registerPlugin(
    'RmsAiSearch',
    'Search',
    'LLL:EXT:rms_ai_search/Resources/Private/Language/locallang_db.xlf:tt_content.CType_rmsaisearch_search',
    'rms_ai_search_icon_png',
    'plugins',
    ExtensionUtility::PLUGIN_TYPE_CONTENT_ELEMENT
);

// Define the new TCA field for aisearch_url_hash
ExtensionManagementUtility::addTCAcolumns('tt_content', [
    'aisearch_url_hash' => [
        'label' => 'LLL:EXT:rms_ai_search/Resources/Private/Language/locallang_db.xlf:tt_content.aisearch_url_hash',
        'config' => [
            'type' => 'input',
            'size' => 30,
            'eval' => 'trim',
        ],
    ],
]);
ExtensionManagementUtility::addTCAcolumns('tt_content', [
    'aisearch_api_host' => [
        'label' => 'LLL:EXT:rms_ai_search/Resources/Private/Language/locallang_db.xlf:tt_content.aisearch_api_host',
        'config' => [
            'type' => 'input',
            'size' => 30,
            'eval' => 'trim',
        ],
    ],
]);
ExtensionManagementUtility::addTCAcolumns('tt_content', [
    'aisearch_prompt_template' => [
        'label' => 'LLL:EXT:rms_ai_search/Resources/Private/Language/locallang_db.xlf:tt_content.aisearch_prompt_template',
        'config' => [
            'type' => 'input',
            'size' => 30,
            'eval' => 'trim',
        ],
    ],
]);
ExtensionManagementUtility::addTCAcolumns('tt_content', [
    'aisearch_list_all_sources' => [
        'label' => 'LLL:EXT:rms_ai_search/Resources/Private/Language/locallang_db.xlf:tt_content.aisearch_list_all_sources',
        'config' => [
            'type' => 'check',
            'default' => 0,
        ],
    ],
]);
ExtensionManagementUtility::addTCAcolumns('tt_content', [
    'aisearch_autofocus_search_field' => [
        'label' => 'LLL:EXT:rms_ai_search/Resources/Private/Language/locallang_db.xlf:tt_content.aisearch_autofocus_search_field',
        'config' => [
            'type' => 'check',
            'default' => 1,
        ],
    ],
]);
ExtensionManagementUtility::addTCAcolumns('tt_content', [
    'aisearch_clear_chat_history' => [
        'label' => 'LLL:EXT:rms_ai_search/Resources/Private/Language/locallang_db.xlf:tt_content.aisearch_clear_chat_history',
        'config' => [
            'type' => 'check',
            'default' => 1,
        ],
    ],
]);

// Show configuration fields for this content element type.
ExtensionManagementUtility::addToAllTCAtypes(
    'tt_content',
    '--div--;Configuration,aisearch_url_hash,aisearch_api_host,aisearch_prompt_template,aisearch_list_all_sources,aisearch_autofocus_search_field,aisearch_clear_chat_history,',
    $contentElementType,
    'after:subheader'
);

// Set preview renderer for the plugin
$GLOBALS['TCA']['tt_content']['types'][$contentElementType]['previewRenderer']
    = \Rmsstuttgart\RmsAiSearch\Preview\SearchPreviewRenderer::class;
